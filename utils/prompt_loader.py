...
#
# ┌─【本文件速览】─────────────────────────────────────────────────────┐
# │ 项目位置：地基层之上的"资源读取层"（无状态，每次调用都读文件）        │
# │ 上游：prompts_conf（提示词路径配置）、path_tool（绝对路径）           │
# │ 下游：react_agent（系统提示词）、rag_service（RAG 总结提示词）、      │
# │       middleware（report 场景切换提示词）                            │
# │ 关键概念：本项目有"双场景 × 三类提示词"共 6 个加载函数：              │
# │   · 场景：robot（默认） / sky（苍穹外卖）                            │
# │   · 类型：main（系统） / rag（RAG总结） / report（报表）             │
# │ 设计要点：★ 不缓存，每次调用都重新读文件 ★                            │
# │   → 好处：改了 prompts/*.txt 立即生效（不必重启）                     │
# │   → 代价：每轮对话可能多次读文件（文本小，可接受）                    │
# │ 错误处理：配置缺键或文件读不到 → 记日志并 raise（不静默吞掉）         │
# └────────────────────────────────────────────────────────────────────┘
