...
#
# ┌─【本文件速览】─────────────────────────────────────────────────────┐
# │ 项目位置：地基层最底层（无任何项目内依赖）                           │
# │ 上游：无                                                            │
# │ 下游：几乎所有读文件的模块（config_handler/logger/file_handler/...) │
# │ 关键概念：用 __file__ 反推项目根，不依赖"启动时的工作目录(cwd)"       │
# │   → 因此无论从哪个目录启动服务，get_abs_path("config/rag.yml") 都对   │
# │ 类比：相当于项目的"GPS 原点"，所有相对路径都从它出发                 │
# └────────────────────────────────────────────────────────────────────┘
